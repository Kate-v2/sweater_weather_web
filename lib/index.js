...
// ---- Session ----
  ...
  function makePostRequest(obj) {
    ...
    req.open(verb, url)
    // req.setRequestHeader("CCONTENT_TYPE", "application/json;charset=UTF-8")
    ...
    // let json = "{'email': 'kate@kate.com', 'password': 'kate', 'password_confirmation': 'kate'}"
    // let json = "{'email': 'kd', 'password': 'kd', 'password_confirmation': 'kd'}"
  ...
  function assessPost(data, display) {
    if (display == 'register') { loginUser(data, true) }
  }

  function loginUser(data, isNewUser=false) {
